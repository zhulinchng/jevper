...
| request fields | `mlflow.spanInputs` always holds every keyword jevper sent. MLflow *also* promotes some of them to attributes, and which ones depends on the route: `logprobs`, `top_logprobs`, `model` on chat; `include`, `store`, `top_logprobs`, `model` on responses; none on the Messages route, where `max_tokens` lives in `span.inputs`. `prompt_cache_key` is in `span.inputs` on both OpenAI routes and is promoted to no attribute, because jevper sends it in `extra_body` — a field of the API, not of any SDK release, so that the `openai` floor in `pyproject.toml` works |
