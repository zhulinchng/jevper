...
| `ProviderError` with 502 naming `exceed_context_size` | The state was longer than the encoder's window, and this backend refuses it rather than cutting it — llama.cpp ignores the `truncate_prompt_tokens` field the server sends, so `clm-serve` relays the encoder's 400 as a 502 | The state, or the window: raise the encoder's context and `clm-serve --max-tokens` **together**. Note `-c` is the total split across slots. This is a permanent failure, so the default policy has now spent all three attempts on it — `RetryPolicy(n_retries=0)` fails it once. See [CLM](local-servers.md#a-long-state-cut-silently-on-vllm-refused-outright-on-llamacpp) |
