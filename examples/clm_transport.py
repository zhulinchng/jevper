"""Reach jevper through a clm.CLMClient you already have.

This is not the ordinary way to use jevper with CLM. clm-serve speaks the System One
wire format directly, so the normal route is an openai client pointed at it:

    SystemOneClient(OpenAI(base_url="http://127.0.0.1:8700/v1", api_key="clm"),
                    model="clm-latest", api="systemone")

See the CLM section of docs/local-servers.md. Use this file when a program already
holds a CLMClient -- because it also calls rank(), or reads CLM_BASE_URL itself --
and does not want to stand up a second client object for the same server.

jevper reaches the System One surface through two low-level methods,
post(path, body=..., cast_to=...) and get(path, cast_to=...), and a CLMClient has
neither. It has the two operations those stand for. This translates one to the
other, and does nothing else.

Three caveats, all from CLMClient's own surface rather than from jevper:

* It needs the private _post, which is the only way to get a raw response body out
  of a CLMClient: its public system_one() answers dataclasses, and jevper reads the
  body as sent, because the answers are the provider's numbers. So this is tied to
  the contrastive-lm version you test it against, and a rename upstream fails loud.
* extra_headers are not plumbed through, because _post takes no headers. Nothing is
  lost: a CLMClient already sends Authorization from CLM_API_KEY, the only header
  this server reads. For any other, set it on the client's own session.
* There is no async twin, because CLMClient is synchronous, so AsyncSystemOneClient
  cannot be built on this. An async program wants the AsyncOpenAI form above.
"""
...
def _with_status(exc: BaseException) -> BaseException:
    ...
    CLMError carries .status; jevper's retry and surface-capability logic reads
    .status_code, which is what both official SDKs set. Left as it is, a 502 from an
    unreachable encoder reaches jevper as an error with no status, and a status is
    what decides whether another attempt is worth making.
    """
...
class CLMTransport:
    ...
    The two methods are the whole contract: post for POST /v1/systemone and get for
    GET /v1/models. cast_to is accepted and not acted on -- both operations answer
    with already-parsed JSON, and jevper asks for a dict, which is what they return.
    """
